...
const GovernmentTeams = () => {
  ...
  const DEPARTMENTS = useMemo(() => {
    return ["All Departments", ...getDepartmentNames()];
  }, []);

  const ORGANIZATIONS = useMemo(() => {
    return ["All Organizations", ...getOrganizationNames()];
  }, []);
  ...
  const loadData = () => {
    ...
    try {
      ...
      console.error("Failed to load government teams:", err);
      setTasks([]);
      setTeamAllocations([]);
    } finally {
      setLoading(false);
  ...
  }, [teams, search, departmentFilter, organizationFilter]);

  const totalEmployees = teams.reduce((sum, t) => sum + t.employees, 0);
  const totalTasks = teams.reduce((sum, t) => sum + t.tasks, 0);
    ...
    <DashboardLayout>
      ...
      <div className="mb-7 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-xs sm:text-sm text-slate-500 font-medium">
            Frontline working groups and execution units across state departments.
          </p>
        </div>
        ...
        <div className="flex items-center gap-2.5">
          ...
          <Button
            variant="primary"
            size="sm"
            onClick={() => window.alert("Team creation interface active.")}
            icon={<Plus size={16} />}
          >
            Register Team
          </Button>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-5 mb-7">
        <StatCard
          ...
          title="Active Teams"
          value={teams.length}
          change="Statewide functional units"
        />
        <StatCard
          title="Frontline Staff"
          value={`${totalEmployees} Staff`}
          change={totalEmployees > 0 ? "Active officers" : "No staff assigned"}
          changeType={totalEmployees > 0 ? "positive" : "neutral"}
        />
        <StatCard
          title="Assigned Tasks"
          value={totalTasks}
          change={totalTasks > 0 ? "In operational queue" : "No active tasks"}
          changeType="neutral"
        />
        <StatCard
          title="Verified Outputs"
          value={totalCompleted}
          change={totalTasks > 0 ? `${overallRate}% verified completion` : "0% completion"}
            ...
            <div>
              ...
              <div className="mt-4 grid grid-cols-3 gap-2 text-center text-xs">
                <div className="p-2.5 rounded-xl bg-[#F4F6F8]">
                  ...
                  <p className="font-extrabold text-slate-900 mt-0.5">{team.tasks}</p>
                </div>
                <div className="p-2.5 rounded-xl bg-[#F4F6F8]">
...
export default GovernmentTeams;