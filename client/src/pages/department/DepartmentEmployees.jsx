...
const DepartmentEmployees = () => {
  ...
  const TEAMS = useMemo(() => ["All Teams", ...getTeamNames()], []);
  ...
  const filteredEmployees = useMemo(() => {
    return employees.filter((emp) => {
      ...
      return matchSearch && matchOrg && matchTeam;
    });
  }, [employees, search, organizationFilter, teamFilter]);
    ...
    <DashboardLayout>
      ...
      <div className="mb-7 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-xs sm:text-sm text-slate-500 font-medium">
            Directory of personnel executing departmental missions across agencies.
          </p>
        </div>
        ...
        <div className="flex items-center gap-2.5">
          ...
          <Button
            variant="primary"
            size="sm"
            onClick={() => window.alert("Personnel enrollment portal active.")}
            icon={<Plus size={16} />}
          >
            Enroll Officer
          </Button>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-5 mb-7">
        <StatCard
          ...
          change="Staff on departmental roster"
        />
        <StatCard
          title="Assigned Tasks"
          value={totalTasks}
          change={totalTasks > 0 ? "In deliverable queue" : "No active tasks"}
          changeType="neutral"
        />
        <StatCard
      ...
      <Card className="mb-7 p-4">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <div className="flex items-center gap-1.5 rounded-full border border-slate-200/90 bg-white px-3 py-1.5 text-xs font-semibold text-slate-700">
              <span className="text-slate-400 text-[11px] uppercase">Team:</span>
              <select
                value={teamFilter}
